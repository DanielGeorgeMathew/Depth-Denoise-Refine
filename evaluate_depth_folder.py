...
root = '~/Downloads/tmp/tmp/'
weights_path = '~/Daniel/DepthRefinement/checkpoints/DDRNet-v2-highNormaldot-lowsmooth'
target_folder = os.path.join(root, 'denoised_depths')
...
n_frames = len(glob(os.path.join(root, 'depth/*.png')))
...
for frame_id in tqdm(range(n_frames)):
    ...
    cv2.imwrite(os.path.join(target_folder, '{}.png'.format(frame_id)), denoised_output.astype(np.uint16))
